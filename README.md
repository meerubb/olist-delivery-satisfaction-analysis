...
MetricOn-Time OrdersLate OrdersShare of orders~92%~8%Average review score (out of 5)4.292.57
...
KPI row — Average Order Value, Total Orders, Total Customers, % Late, Average Review Score
Category & geographic breakdown — late delivery rate by product category (filtered to a minimum order count to avoid small-sample distortion), and a scatter plot of late rate vs. order volume by state
Trend & satisfaction link — monthly trend of late delivery rate, and a direct comparison of average review score for on-time vs. late orders, plus review score by category
...
During development, a DAX DATEDIFF-based late-delivery measure was found to return an incorrect result (~11% instead of the verified ~8.1%) due to how the function evaluated the date comparison in this model. This was caught by independently verifying the percentage against the raw dataset in Python/pandas, and resolved by switching to direct date comparison logic in DAX. All final figures in this dashboard and the accompanying report are independently verified against the source data.
